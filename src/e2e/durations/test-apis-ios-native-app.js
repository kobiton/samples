...
import {testiOSNativeApp} from '../core/iOS-native-app-test.js'

const {shortRunLoop} = getConfig()
let onlineDevices
let server

describe('test short iOS native app on one device', () => {
  ...
  for (let i = 0; i < shortRunLoop; i++) {
    it(`should run test [${i + 1}/${shortRunLoop}]`, async() => {
      ...
      const results = await testiOSNativeApp(server, onlineDevices)
